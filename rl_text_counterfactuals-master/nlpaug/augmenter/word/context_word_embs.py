...
class ContextualWordEmbsAug(WordAugmenter):
    ...
    def substitute(self, tokens, aug_idxes=[]):
        debug = False
        # tokens is a list of words to augment
        # aug_idxes is a list with a single token position to consider to change

        aug_idx = aug_idxes[0]

        #if word is unknown (OOV) or in stop words, don't substitute it 
        # and return ["SKIP"] or some message to counterfac_env so it knows not to treat this as a true subsitute (ie, don't penalize)
        #valid_idxes = self._get_aug_idxes(tokens)
        # HERE THIS IS WAY TOO AGGRESSIVE IN KICKING SHIT OUT!
        #print(valid_idxes)
        
        """ SKIP NOT REPLACING UNKNOWN WORDS..  blasphemous invalid word
        word_idxes = self.pre_skip_aug(tokens)
        valid_idxes = self.skip_aug(word_idxes, tokens)
        if aug_idx not in valid_idxes:
            if debug:
                print("UNKNOWN OR STOPWORD at id",aug_idx,tokens[aug_idx],"invalid word for sub so skip")
            return [("INVALID WORD FOR SUB",0)] 
        """
        ...
        for _ in range(retry_cnt):
            ...
            candidates = self.model.predict(masked_text, target_word=original_word, n=10+_)   # THIS IS WHERE WE SET THE NUMBER OF CANDIDATES ( 10 words )
            if len(candidates) > 0:
                can_ps = [c[1] for c in candidates]
                can_probs = [c / sum(can_ps) for c in can_ps]
                can_words = [c[0] for c in candidates] 
                can_ind =  np.random.choice(len(candidates), p=can_probs)
                substitute_word, prob =  can_words[can_ind], can_probs[can_ind]
                break
            else:
                if debug:
        ...
        if debug:
            print(len(tokens),candidates)
            if aug_idx > 5 and (aug_idx + 5) < len(tokens):
                print("SUBBINGa",original_word,"in",tokens[(aug_idx - 5):(aug_idx + 5)],"curid:",aug_idx,"most likely with",substitute_word,"with prob",prob)
            elif aug_idx > 5:
                print("SUBBINGb",original_word,"in",tokens[(aug_idx - 5):len(tokens)],"curid:",aug_idx,"most likely with",substitute_word,"with prob",prob)
            elif (aug_idx + 5) < len(tokens):
                print("SUBBINGc",original_word,"in",tokens[0:(aug_idx + 5)],"curid:",aug_idx,"most likely with",substitute_word,"with prob",prob)
            else:
                print("SUBBINGd",original_word,"in",tokens,"curid:",aug_idx,"most likely with",substitute_word,"with prob",prob)
        return candidates

    def substitute_v2(self, data, aug_idxes=[]):
        # data is the string to augment
        # aug_idxes is a list with a single token position to consider to change
        ...
        for i, aug_idx in enumerate(aug_idxes):
            ...
            for _ in range(retry_cnt):
                ...
                candidates = self.model.predict(masked_text, target_word=original_word, n=10+_)

                if len(candidates) > 0:
                    #print("AUG_IDX", aug_idx,"Data",data,"Candidates",candidates)
                    # AUG_IDX 0 Data The quick brown fox jumps over the lazy dog . 
                    # Candidates [('a', 0.7633162), ('big', 0.00012241643), ('two', 0.00022471338), ('one', 0.003609798), 
                    #             ('"', 8.375053e-05), ('my', 0.00075338356), ('no', 0.000353659), ('some', 0.0023645868), ('any', 1.5138334e-05), ('his', 0.0006432617)]
                    ...
                    can_ind =  np.random.choice(len(candidates), p=can_probs)
                    substitute_word, prob =  can_words[can_ind], can_probs[can_ind]
                    #print("Word chosen:", can_ind, substitute_word, prob)
                    break
                else:
                    print("NO CANDIDATES FOUND!")
