...
class GLEU :
    ...
    def load_sources(self,spath) :
        self.all_s_ngrams = [ [ self.get_ngram_counts(line.split(),n) for n in range(1,self.order+1) ] for line in open(spath) ]
    ...
    def gleu_stats(self,i,r_ind=None):
      ...
      for n in xrange(1,self.order+1):
        ...
        #print("H",len(h_ngrams),"S",len(s_ngrams),"R",len(r_ngrams)) #('H', 14, 'S', 13, 'R', 11)
    ...
    def gleu(self,stats,smooth=False):
        ...
        #print(stats, smooth)  #0 ([15, 11, 4, 15, 3, 14, 2, 13, 0, 12], True)
        if smooth :
            stats = [ s if s != 0 else 1 for s in stats ]
        #print(stats) #[15, 11, 4, 15, 3, 14, 2, 13, 1, 12]
        ...
        #print(log_gleu_prec)  #-1.8047274269

        ret = math.exp(min([0, 1-float(r)/c]) + log_gleu_prec)
        #print(ret) #0.164519293999
        return ret
